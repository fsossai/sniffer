...
int main(int argc, char *argv[])
{
	...
	for (i = 1; i<argc; i++)
	{
		if (!strcmp(argv[i], "-f"))
		{
			if (set_filters( (i+1 < argc) ? argv[i+1] : NULL ) < 0)
				return -1;
		}
...
int set_filters(char *llevel)
{
	...
	filters[filters_counter].ulevel_name = (char*)malloc(filters[filters_counter].ulen);
	...
	filters[filters_counter].llevel_name = (char*)malloc(filters[filters_counter].llen);
	strcpy(filters[filters_counter].llevel_name,llevel);

	printf("Filter all '%s' that carry '%s' data\n",filters[filters_counter].llevel_name,filters[filters_counter].ulevel_name);
...
void showf(struct eth_frame *eth, char *outp)
{
	...
	for (i = 0; i<filters_counter || all; i++)
	{
		...
		if (start <= end || all)
		{
			if (all)
			{
				for (j = strlen(outp) -1; (j>=0) && (outp[j] != '.'); j--);
				cur = &outp[j+1];
			}
			else

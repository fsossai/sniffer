...
void print_ip(struct ip_datagram *ip)
{
	...
	printf("Header checksum: 0x%.2X 0x%.2X\n",(ip->checksum & 0xFF00)>>8,ip->checksum & 0x00FF);
	printf("Source address: %i.%i.%i.%i\n",
		((unsigned char*)&ip->src)[0],
		((unsigned char*)&ip->src)[1],
		((unsigned char*)&ip->src)[2],
		((unsigned char*)&ip->src)[3]);
	printf("Destination address: %i.%i.%i.%i\n",
		((unsigned char*)&ip->dst)[0],
		((unsigned char*)&ip->dst)[1],
		((unsigned char*)&ip->dst)[2],
		((unsigned char*)&ip->dst)[3]);
	printf("\n");		  
}
